...
| `cartstorage`         | `cartstorage/`         | Cart key-value store backed by Supabase (`tinyfaas_webshop_cart` table) |
...
> **Note:** `payment`, `getproduct`, `searchproducts` are part of the reference architecture but is not called by the current
...
frontend (entry, dispatches by operation)
  ...
  operation="get"
    ...
    |-sync-> getads
    |-sync-> getcart
    |          |-sync-> cartstorage  [Supabase: SELECT tinyfaas_webshop_cart WHERE user_id=?]
  ...
  operation="addcart"
    |-sync->  addcartitem
    |           |-sync->  cartstorage  [Supabase: UPSERT INTO tinyfaas_webshop_cart]
    |-sync->  getcart
                |-sync-> cartstorage  [Supabase: SELECT tinyfaas_webshop_cart WHERE user_id=?]
  ...
  operation="checkout"
    |-sync->  checkout
                ...
                |-async-> emptycart
                            |-async-> cartstorage  [Supabase: DELETE FROM tinyfaas_webshop_cart WHERE user_id=?]
...
- A Supabase project with the `tinyfaas_webshop_cart` table created (see below)
...
bash tests/supabase/scripts/db.sh create webshop tinyfaas
```
...
This creates `public.tinyfaas_webshop_cart` with the composite primary key, indexes, RLS enabled,
...
```bash
bash tests/supabase/scripts/db.sh reset webshop tinyfaas
```

To inspect all stored cart rows:

```bash
bash tests/supabase/scripts/db.sh get webshop tinyfaas
```

To drop the table entirely:

```bash
bash tests/supabase/scripts/db.sh destroy webshop tinyfaas
```
